	...
	public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) 
	{
		...
		if(sender instanceof Player) 
		{
			...
			/*if(args.length == 0) 
			{
				...
				//getExecutor("example").onCommand(sender, cmd, commandLabel, args);
				getExecutor("cityworld").onCommand(sender, cmd, commandLabel, args);
				player.sendMessage("Command is " + cmd + "!");
				return true;
			}*/ //Removed, was only used for a base command, which i do not want for this plugin.
			...
			if(args.length > 0) 
			{
				...
				} 
				{
					...
					sender.sendMessage("The command " + args[0] + " doesn't exist!");
					return true;
				}
			}
		} 
